...
class MerkleTree {
     ...
     */
    constructor(leaves: (Buffer | string)[], hashFn: (data: Buffer) => Buffer = defaultHash) {
        ...
        this._leaves = leaves.map((l, index) => {
            try {
                ...
                throw new MerkleTreeError(`Failed to hash leaf at index ${index}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
        this.hashFn = hashFn;
    ...
    getProof(index: number): ProofItem[] {
        ...
        if (index < 0 || index >= this._leaves.length) {
            throw new MerkleTreeError(`Leaf index ${index} out of bounds [0, ${this._leaves.length - 1}]`);
        }
    ...
    static verifyProof(
        ...
        try {
            return proof.reduce((computedHash, { sibling, position }) => {
                if (!Buffer.isBuffer(sibling)) {
                    throw new MerkleTreeError('Proof sibling must be a Buffer');
                }
                if (position !== 'left' && position !== 'right') {
                    throw new MerkleTreeError('Proof position must be either "left" or "right"');
                }
                if (position === 'left') {
            ...
            }, leafHash).equals(root);
        } catch (error) {
            throw new MerkleTreeError(`Proof verification failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
     ...
     */
    static fromJSON(json: string, hashFn: (data: Buffer) => Buffer = defaultHash): MerkleTree {
        try {
            ...
            throw new MerkleTreeError(`Failed to deserialize tree: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
if (require.main === module) {
    ...
    proof.forEach((item, i) => {
        console.log(`  Sibling ${i} (${item.position}):`, item.sibling.toString('hex'));
    });
